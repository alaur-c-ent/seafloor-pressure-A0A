...
This script use fonctions of the GitHub reposistory
...
6. Model the drift with least mean square inversion follonwing A. Duvernay method
...
mission = 'GEODAMS'
station_name = 'SEIR'
rsk_reference = '{serialnumber}_{recovery_date_YYYYMMDD}_{recovery_time_HHMM}'
...
resample_to = '1min'
data_path = os.path.join(root_path, mission, rsk_reference, f'{station_name}_OBP_{resample_to}.csv')

events_path =  os.path.join(root_path, mission, rsk_reference, f'{rsk_reference}_events.txt')

zeros_path = os.path.join(root_path, mission, rsk_reference, f'{station_name}_zeros.csv')
...
window = pd.Timedelta(seconds=20*60) # also working with datetime timedelta
